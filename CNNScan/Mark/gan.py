...
class AutoEncoder(nn.Module):
	def __init__(self, config):
		...
		self.decoder = MarkGenerator(config, config['gen_seed_len'])
	def forward(self, images):
...
def train_autoencoder(config, model, train_loader, test_loader):
	def iterate_once(config, model, loader, criterion, do_train=False, optimizer=None):
	...
	for epoch in range(config['epochs']):
		...
		model.train()
		optimizer.zero_grad()
		count, loss = iterate_once(config, model, train_loader, criterion, True, optimizer)
